const sapxepTen=(th)=>{
    ...
    if(co=="true"){
        // Tăng
        danhsachloc=dsPhukien.sort(function(a, b){return (a.Ten.localeCompare(b.Ten))});
        th.innerHTML=`Sắp xếp Tên Giảm`
        th.setAttribute("sxTen",false)
    }else{
        // Giảm
        danhsachloc=dsPhukien.sort(function(a, b){return (b.Ten.localeCompare(a.Ten))});
        th.innerHTML=`Sắp xếp Tên Tăng`
        th.setAttribute("sxTen",true)
    }
    Xuat_Phukien(dsPhukien,Th_Phukien);
}
const sapxepGia=(th)=>{
    ...
    if(co=="true"){
        // Tăng
        danhsachloc =dsPhukien.sort(function(a, b){return Number(a.Don_gia_Ban) - Number(b.Don_gia_Ban)});
        th.innerHTML=`Sắp xếp Giá Giảm`
        th.setAttribute("sxGia",false)
    }else{
        // Giảm
        danhsachloc =dsPhukien.sort(function(a, b){return Number(b.Don_gia_Ban) - Number(a.Don_gia_Ban)});
...
const taoNhom = () => {
    dsNhom = Array.from(new Set(dsPhukien.map(x => x.Nhom_Mat_hang.Ma_so))).map(Ma_so => {
        nhom = {
            Ma_so: Ma_so,
            Ten: dsPhukien.find(x => x.Nhom_Mat_hang.Ma_so == Ma_so).Nhom_Mat_hang.Ten
        }
        return nhom
    })
...
const XuatNhom = (dsNhom, The_hien) => {
    ...
    dsNhom.forEach(nhom => {
        html += `
        <button class="btn btn-sm btn-primary" id='${nhom.Ma_so}' onclick=XuatPhukienTheoNhom('${nhom.Ma_so}')>${nhom.Ten}</button>
        `
    })
    The_hien.innerHTML = html;
}

const XuatPhukienTheoNhom = (Ma_nhom) => {
    ...
    if (Ma_nhom == "ALL") {
        ...
        dsPhukien=dsGoc.filter(phukien => phukien.Nhom_Mat_hang.Ma_so == Ma_nhom);
    }
    Xuat_Phukien(dsPhukien, Th_Phukien)
}

const Xuat_Phukien=(ds,tag)=>{
    let html=``;
    ds.forEach(item=>{
        html+=`
        <div class="col mb-5">
...
const KeyCode=(event)=>{
    if (event.keyCode == 13) {
        ...
        console.log(gt)
        dsPhukien = dsGoc.filter(x => x.Ten.toLowerCase().includes(gt.toLowerCase()))
        Xuat_Phukien(dsPhukien, Th_Phukien)
    }
}